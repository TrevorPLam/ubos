...
export const projectTemplates = pgTable(
  ...
  (table) => [index("idx_templates_org").on(table.organizationId)],
);
...
export const milestones = pgTable(
  ...
  (table) => [index("idx_milestones_project").on(table.projectId)],
);
...
export const invoiceSchedules = pgTable(
  ...
  (table) => [index("idx_schedules_engagement").on(table.engagementId)],
);
...
export const invoices = pgTable(
  "invoices",
  {
    ...
    engagementId: varchar("engagement_id")
      .references(() => engagements.id, { onDelete: "cascade" })
      .notNull(),
    scheduleId: varchar("schedule_id").references(() => invoiceSchedules.id, {
      onDelete: "set null",
    }),
    clientCompanyId: varchar("client_company_id").references(() => clientCompanies.id, {
      onDelete: "set null",
...
export const vendors = pgTable(
  ...
  (table) => [index("idx_vendors_org").on(table.organizationId)],
);
...
export const insertFileObjectSchema = createInsertSchema(fileObjects).omit({
  id: true,
  createdAt: true,
});
...
export const insertInvoiceScheduleSchema = createInsertSchema(invoiceSchedules).omit({
  id: true,
  createdAt: true,
});
