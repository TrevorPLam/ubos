...
function main() {
  console.log('🔍 Scanning agent roles for task indices...');
  ...
  console.log(`📁 Found ${roles.length} roles: ${roles.slice(0, 5).join(', ')}${roles.length > 5 ? '...' : ''}`);
  ...
  console.log('📝 Generating consolidated task index...');
  const markdown = generateMarkdown(roleTasks);

  // Write to output file
  writeFileSync(OUTPUT_FILE, markdown, 'utf-8');
  
  console.log(`✅ Generated TASK_INDEX.md with ${totalTasks} tasks from ${roles.length} roles`);
}
