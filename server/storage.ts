...
export interface IStorage {
  ...
  updateProposal(id: string, orgId: string, data: Partial<InsertProposal>): Promise<Proposal | undefined>;
  ...
  updateEngagement(id: string, orgId: string, data: Partial<InsertEngagement>): Promise<Engagement | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getClientCompanies(orgId: string): Promise<ClientCompany[]> {
    return db.select().from(clientCompanies).where(eq(clientCompanies.organizationId, orgId)).orderBy(desc(clientCompanies.createdAt));
  }

  async getClientCompany(id: string, orgId: string): Promise<ClientCompany | undefined> {
    const [client] = await db.select().from(clientCompanies).where(and(eq(clientCompanies.id, id), eq(clientCompanies.organizationId, orgId)));
    return client;
  }
  ...
  async deleteClientCompany(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(clientCompanies).where(and(eq(clientCompanies.id, id), eq(clientCompanies.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getContacts(orgId: string): Promise<Contact[]> {
    return db.select().from(contacts).where(eq(contacts.organizationId, orgId)).orderBy(desc(contacts.createdAt));
  }

  async getContact(id: string, orgId: string): Promise<Contact | undefined> {
    const [contact] = await db.select().from(contacts).where(and(eq(contacts.id, id), eq(contacts.organizationId, orgId)));
    return contact;
  }
  ...
  async updateContact(id: string, orgId: string, data: Partial<InsertContact>): Promise<Contact | undefined> {
    const [contact] = await db.update(contacts).set({ ...data, updatedAt: new Date() }).where(and(eq(contacts.id, id), eq(contacts.organizationId, orgId))).returning();
    return contact;
  }

  async deleteContact(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(contacts).where(and(eq(contacts.id, id), eq(contacts.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getDeals(orgId: string): Promise<Deal[]> {
    return db.select().from(deals).where(eq(deals.organizationId, orgId)).orderBy(desc(deals.createdAt));
  }

  async getDeal(id: string, orgId: string): Promise<Deal | undefined> {
    const [deal] = await db.select().from(deals).where(and(eq(deals.id, id), eq(deals.organizationId, orgId)));
    return deal;
  }
  ...
  async updateDeal(id: string, orgId: string, data: Partial<InsertDeal>): Promise<Deal | undefined> {
    const [deal] = await db.update(deals).set({ ...data, updatedAt: new Date() }).where(and(eq(deals.id, id), eq(deals.organizationId, orgId))).returning();
    return deal;
  }

  async deleteDeal(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(deals).where(and(eq(deals.id, id), eq(deals.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getProposals(orgId: string): Promise<Proposal[]> {
    return db.select().from(proposals).where(eq(proposals.organizationId, orgId)).orderBy(desc(proposals.createdAt));
  }

  async getProposal(id: string, orgId: string): Promise<Proposal | undefined> {
    const [proposal] = await db.select().from(proposals).where(and(eq(proposals.id, id), eq(proposals.organizationId, orgId)));
    return proposal;
  }
  ...
  async updateProposal(id: string, orgId: string, data: Partial<InsertProposal>): Promise<Proposal | undefined> {
    const [proposal] = await db.update(proposals).set({ ...data, updatedAt: new Date() }).where(and(eq(proposals.id, id), eq(proposals.organizationId, orgId))).returning();
    return proposal;
  }

  async deleteProposal(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(proposals).where(and(eq(proposals.id, id), eq(proposals.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getContracts(orgId: string): Promise<Contract[]> {
    return db.select().from(contracts).where(eq(contracts.organizationId, orgId)).orderBy(desc(contracts.createdAt));
  }
  ...
  async updateContract(id: string, orgId: string, data: Partial<InsertContract>): Promise<Contract | undefined> {
    const [contract] = await db.update(contracts).set({ ...data, updatedAt: new Date() }).where(and(eq(contracts.id, id), eq(contracts.organizationId, orgId))).returning();
    return contract;
  }

  async deleteContract(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(contracts).where(and(eq(contracts.id, id), eq(contracts.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getEngagements(orgId: string): Promise<Engagement[]> {
    return db.select().from(engagements).where(eq(engagements.organizationId, orgId)).orderBy(desc(engagements.createdAt));
  }

  async getEngagement(id: string, orgId: string): Promise<Engagement | undefined> {
    const [engagement] = await db.select().from(engagements).where(and(eq(engagements.id, id), eq(engagements.organizationId, orgId)));
    return engagement;
  }
  ...
  async updateEngagement(id: string, orgId: string, data: Partial<InsertEngagement>): Promise<Engagement | undefined> {
    const [engagement] = await db.update(engagements).set({ ...data, updatedAt: new Date() }).where(and(eq(engagements.id, id), eq(engagements.organizationId, orgId))).returning();
    return engagement;
  }

  async deleteEngagement(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(engagements).where(and(eq(engagements.id, id), eq(engagements.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getProjects(orgId: string): Promise<Project[]> {
    return db.select().from(projects).where(eq(projects.organizationId, orgId)).orderBy(desc(projects.createdAt));
  }

  async getProject(id: string, orgId: string): Promise<Project | undefined> {
    const [project] = await db.select().from(projects).where(and(eq(projects.id, id), eq(projects.organizationId, orgId)));
    return project;
  }
  ...
  async updateProject(id: string, orgId: string, data: Partial<InsertProject>): Promise<Project | undefined> {
    const [project] = await db.update(projects).set({ ...data, updatedAt: new Date() }).where(and(eq(projects.id, id), eq(projects.organizationId, orgId))).returning();
    return project;
  }

  async deleteProject(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(projects).where(and(eq(projects.id, id), eq(projects.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getTasks(orgId: string, projectId?: string): Promise<Task[]> {
    if (projectId) {
      return db.select().from(tasks).where(and(eq(tasks.organizationId, orgId), eq(tasks.projectId, projectId))).orderBy(desc(tasks.createdAt));
    }
  ...
  async updateTask(id: string, orgId: string, data: Partial<InsertTask>): Promise<Task | undefined> {
    const [task] = await db.update(tasks).set({ ...data, updatedAt: new Date() }).where(and(eq(tasks.id, id), eq(tasks.organizationId, orgId))).returning();
    return task;
  }

  async deleteTask(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(tasks).where(and(eq(tasks.id, id), eq(tasks.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getThreads(orgId: string): Promise<Thread[]> {
    return db.select().from(threads).where(eq(threads.organizationId, orgId)).orderBy(desc(threads.lastMessageAt));
  }

  async getThread(id: string, orgId: string): Promise<Thread | undefined> {
    const [thread] = await db.select().from(threads).where(and(eq(threads.id, id), eq(threads.organizationId, orgId)));
    return thread;
  }
  ...
  async createMessage(data: InsertMessage): Promise<Message> {
    ...
    await db.update(threads).set({ lastMessageAt: new Date() }).where(eq(threads.id, data.threadId));
    return message;
  }
  ...
  async getInvoice(id: string, orgId: string): Promise<Invoice | undefined> {
    const [invoice] = await db.select().from(invoices).where(and(eq(invoices.id, id), eq(invoices.organizationId, orgId)));
    return invoice;
  }
  ...
  async updateInvoice(id: string, orgId: string, data: Partial<InsertInvoice>): Promise<Invoice | undefined> {
    const [invoice] = await db.update(invoices).set({ ...data, updatedAt: new Date() }).where(and(eq(invoices.id, id), eq(invoices.organizationId, orgId))).returning();
    return invoice;
  }

  async deleteInvoice(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(invoices).where(and(eq(invoices.id, id), eq(invoices.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }

  async getBills(orgId: string): Promise<Bill[]> {
    return db.select().from(bills).where(eq(bills.organizationId, orgId)).orderBy(desc(bills.createdAt));
  }

  async getBill(id: string, orgId: string): Promise<Bill | undefined> {
    const [bill] = await db.select().from(bills).where(and(eq(bills.id, id), eq(bills.organizationId, orgId)));
    return bill;
  }
  ...
  async updateBill(id: string, orgId: string, data: Partial<InsertBill>): Promise<Bill | undefined> {
    const [bill] = await db.update(bills).set({ ...data, updatedAt: new Date() }).where(and(eq(bills.id, id), eq(bills.organizationId, orgId))).returning();
    return bill;
  }

  async deleteBill(id: string, orgId: string): Promise<boolean> {
    const result = await db.delete(bills).where(and(eq(bills.id, id), eq(bills.organizationId, orgId)));
    return (result.rowCount ?? 0) > 0;
  }
  ...
  async getActivityEvents(orgId: string, engagementId?: string): Promise<ActivityEvent[]> {
    if (engagementId) {
      return db.select().from(activityEvents).where(and(eq(activityEvents.organizationId, orgId), eq(activityEvents.engagementId, engagementId))).orderBy(desc(activityEvents.createdAt));
    }
    return db.select().from(activityEvents).where(eq(activityEvents.organizationId, orgId)).orderBy(desc(activityEvents.createdAt));
  }
}
